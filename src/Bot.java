...
public class Bot {
    ...
    //PADROES DE CORES
    Color[] sequencia1 = {pretoTela, pretoTela, pretoTela}; //APOSTA VERMELHO
    Color[] sequencia2 = {vermelhoTela, vermelhoTela, vermelhoTela}; //APOSTA PRETO


    Color[] sequencia3 = {vermelhoTela, pretoTela, vermelhoTela, pretoTela, vermelhoTela}; //APOSTA PRETO
    Color[] sequencia4 = {vermelhoTela, vermelhoTela, vermelhoTela}; //APOSTA PRETO
...
//        verificaSorteadas();
        ...
        while (flag == 0) {
           ...
           todasCoresParaEscolherAposta = verificaValorAposta(valorAposta, todasCoresParaEscolherAposta, apostaAleatoria);
            Color[] cincoUltasCores = getColors();
//----------------------------------------------------------------------------------------------------------------------
            //VERIFICA SEQUENCIA1
            ...
            if (cincoUltasCores[0].equals(pretoTela) && cincoUltasCores[1].equals(pretoTela) && cincoUltasCores[2].equals(pretoTela)) {
                        valorAposta = valorAposta * 4;
                        corEscolhida = vermelhoTela;
                        todasCoresParaEscolherAposta = vermelho;

            }
            if (cincoUltasCores[0].equals(vermelhoTela) && cincoUltasCores[1].equals(vermelhoTela) && cincoUltasCores[2].equals(vermelhoTela)) {
                            valorAposta = valorAposta * 4;
...
//                        System.out.println("cor teste"+corResultado);
                    ...
                    //COMO O VALOR DA APOSTA E MENOR DO QUE 20 ELE NAO APOSTA NAS BRANCAS
                    else if (valorAposta < 20) {
                        //VERIFICA SE APOSTA == COR RESULTADO (PRETO OU VERMELHO)
                        if (corEscolhida.equals(corResultado)) {
                            saldo = saldo + (xPreto * valorAposta) - valorAposta;//SE SUBTRAI O VALOR APOSTADO NO BRANCO
                            System.out.println("Cor resultado: " + re + "Acertou: R$" + saldo);
                        }
                        //VERIFICA SE APOSTA != RESULTADO (PRETO OU VERMELHO)
                        else if (!corEscolhida.equals(corResultado)) {
                            saldo = (saldo - valorAposta) - valorAposta;
